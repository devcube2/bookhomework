...
public interface BookMapper {
    @Insert("insert into 책추천(제목, 저자, 소개) values (#{제목}, #{저자}, #{소개})")
    boolean 책등록(BookDto bookDto);

    @Update("update 책추천 set 제목 = #{제목}, 저자 = #{저자}, 소개 = #{소개} where 번호 = #{번호}")
    boolean 책수정(BookDto bookDto);

    @Delete("delete from 책추천 where 번호 = #{번호}")
    boolean 책삭제(int 번호);
